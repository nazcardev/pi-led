...
def light_up_all_red():
    ...
    plasma.set_all(0, 0, 0, brightness=1.0) # Turn all lights off
    plasma.show()
...
def countdown_sequence():
    ...
    plasma.set_all(0, 0, 255, brightness=1.0) # Blue
    plasma.show()
    time.sleep(COUNTDOWN_FLASH_DURATION * 2)
    plasma.set_all(0, 0, 0, brightness=1.0)
...
def light_up_pixel_group(light_index, color):
    ...
    start_pixel, end_pixel = get_pixel_indices_for_light(light_index)
    for i in range(start_pixel, end_pixel):
        plasma.set_pixel(i, color[0], color[1], color[2], brightness=1.0)
    plasma.show()
...
try:
    ...
    while True:
        ...
        plasma.set_all(255, 255, 255, brightness=1.0)
        plasma.show()
